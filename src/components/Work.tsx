...
const SmallWorkCard: React.FC<CardProps> = ({ data }) => {
  return (
    <Box
      borderRadius={1}
            ...
            <Link href={data.source_url || undefined}>
              <GitHub
                sx={{
                  width: 40,
                  height: 40,
                  color: data.source_url ? "black" : "#ccc",
                }}
              />
            </Link>
          </Tooltip>
            ...
            <Link href={data.download_url || undefined}>
              <GetApp
                sx={{
                  width: 40,
                  height: 40,
                  color: data.download_url ? "black" : "#ccc",
                }}
              />
            </Link>
          </Tooltip>
  ...
  return (
    ...
    >
      <Box
        ...
        width={(140 / 9) * 16}
        maxWidth="30%"
        sx={{ backgroundColor: "#ccc", aspectRatio: "16/9" }}
      >
        <Box
          component="img"
          src={data.thumbnail}
          height="100%"
          width="100%"
          sx={{ objectFit: "contain", verticalAlign: "top" }}
        />
      </Box>
      <Box flexGrow={1}>
        <Typography
        ...
        <Box display="flex" gap={2} justifyContent="flex-end">
          <Tooltip title={data.url && "サイトを開く"}>
            <Link href={data.url || undefined}>
              <OpenInBrowser
                sx={{ ...iconSize, color: data.url ? "black" : "#ccc" }}
              />
            </Link>
          </Tooltip>
          <Tooltip title={data.source_url && "コードを見る"}>
            <Link href={data.source_url || undefined}>
              <GitHub
                sx={{ ...iconSize, color: data.source_url ? "black" : "#ccc" }}
              />
            </Link>
          </Tooltip>
          <Tooltip title={data.download_url && "アプリを手に入れる"}>
...
const WorkCard: React.FC<WorkProps> = ({ data, isUpSmall, isUpMedium }) => {
  return (
    <>
      {isUpSmall ? (
